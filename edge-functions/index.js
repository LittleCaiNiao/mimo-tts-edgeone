...
function getConfigUrl(){var c=getConfig();var p=new URLSearchParams({api_key:c.apiKey,voice:c.model==="v2.5_design"?c.voiceDesc:c.voice,model:c.model});return "${base_url}/api/legado-import?"+p.toString()}
function getLegadoUrl(){return "legado://import/httpTTS?src="+encodeURIComponent(getConfigUrl())}
function doImport(){var c=getConfig();if(!c.apiKey)return showStatus("importStatus","请先输入 API Key",false);window.location.href=getLegadoUrl();showStatus("importStatus","正在唤起 legado...",true)}
async function copyUrl(){var c=getConfig();if(!c.apiKey)return showStatus("importStatus","请先输入 API Key",false);var u=getConfigUrl();try{await navigator.clipboard.writeText(u);showStatus("importStatus","链接已复制！粘贴到 legado 朗读引擎 → 网络导入",true)}catch(e){var ta=document.createElement("textarea");ta.value=u;document.body.appendChild(ta);ta.select();document.execCommand("copy");document.body.removeChild(ta);showStatus("importStatus","链接已复制！粘贴到 legado 朗读引擎 → 网络导入",true)}}
function showQR(){var c=getConfig();if(!c.apiKey)return showStatus("importStatus","请先输入 API Key",false);var u=getLegadoUrl();document.getElementById("qrSection").style.display="block";document.getElementById("qrImg").src="https://api.qrserver.com/v1/create-qr-code/?size=200x200&data="+encodeURIComponent(u);showStatus("importStatus","扫码导入链接已生成",true)}
</script>
</body>
</html>`;
